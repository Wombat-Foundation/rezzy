...
/// [`LeanEvent::validate_syntactic`](rz_core::LeanEvent::validate_syntactic)
...
pub fn report_gaps<F>(
    ...
    Vec<rz_core::state::MissingAuthEvent<String>>,
)
where
    F: Fn(&String) -> bool,
{
    let backward = rz_core::find_backward_extremities(events_map, &exists);
    let missing_auth = rz_core::find_missing_auth_events(events_map, &exists);
...
pub fn resolve_parent_states(
    ...
    let bare_maps: Vec<ResolvedState> = parent_states
        ...
        .collect();
    let resolved = rz_core::resolve_state_maps(&bare_maps, &filtered_context, version);
...
pub fn partition_and_resolve_state(
    ...
    let (unconflicted_state, conflicted_state_set) =
        rz_core::partition_state_maps(state_maps, state_maps.len());
    ...
    if version == StateResVersion::V2_1 || version == StateResVersion::V2_1_1 {
        let subgraph = rz_core::compute_v2_1_conflicted_subgraph(events_map, &conflicted_state_set);
    ...
    let final_state_map = rz_core::resolve_iterative_sort(
        &unconflicted_state,
        &conflicted_events,
        events_map,
        version,
...
pub fn apply_global_power_levels(
    ...
    let sorted_power_ids =
        rz_core::lean_kahn_sort(&power_events, events_map, create_ev, version, &mut pl_cache);
...
{"event_id":"C","type":"m.room.message","sender":"@x:x","depth":3,"prev_events":["B"],"auth_events":["A","B"]}
        ...
        assert_eq!(
            backward,
            [] as [rz_core::BackwardExtremity<std::string::String>; 0]
        );
        assert_eq!(
            missing_auth,
            [] as [rz_core::MissingAuthEvent<std::string::String>; 0]
        );
    }
}
