...
use rz_core::{resolved_state_entries, LeanEvent, StateResVersion};
...
pub struct FormattingContext<'a> {
    ...
    pub auth_graph: &'a rz_core::auth::roaring::AuthGraph,
...
pub fn compute_component_roots(
    ...
    if !events_map.is_empty() {
        ...
        let id_to_index = rz_core::index_by_event_id(index_to_ev.iter().copied());
