...
| 1.2        | **m.room.create**: reject if event has `room_id` (V12: room_id is event_id with `!`)                  | V12      | [ ]   | Not implemented — the V12 requirement (reject if an event carries `room_id`; V12 `room_id` is the event_id prefixed with `!`) is not enforced. `LeanEvent.room_id` is an opt-in `Option<RoomId>`; no check compares it against an accepted `m.room.create` event ID.                                                                                                                                     |
