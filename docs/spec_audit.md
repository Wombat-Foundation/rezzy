...
| 1.2        | **m.room.create**: reject if event has `room_id` (V12: room_id is event_id with `!`)                  | V12      | [x]   | `check_auth_chain` (`LeanEvent.room_id` is opt-in; when present on a V12+ create event, it must equal `!` + the event's own hash-derived event ID, else `InvalidSyntax`). Lives in `check_auth_chain` rather than the generic `check_auth_with_context`, since `room_id` is a concrete `LeanEvent` field, not exposed by the `EventLike` trait (same reason Rule 2.5, below, lives there too).           |
...
| 2 (V12)    | Reject if `room_id` is not an accepted `m.room.create` event ID                                       | V12      | [ ]   | Not implemented — the V12 requirement is not enforced. `LeanEvent.room_id` is `Option<RoomId>` populated only by trusted ingest-time tagging, so the check is opt-in/never fires for `None`; nothing validates that `room_id` equals an accepted `m.room.create` event ID.                                                                                                                               |
