...
use rz_core::auth::{check_auth_with_context, RoomState};
...
fn test_derive_auth_events_rejects_if_auth_event_is_rejected() {
    ...
    let mut state = rz_core::state::at::SharedState::new();
    state.insert(
    ...
    assert_eq!(
        err,
        rz_core::auth::AuthError::RejectedAuthEvent {
...
fn room_state_from_shared(
    shared: &rz_core::state::at::SharedState<String, String>,
