    ...
    for version in [StateResVersion::V2_1, StateResVersion::V2_1_1] {
        ...
        assert!(
            resolved.contains_key(&(
                rz_core::basespec::event_types::EventType::from("m.room.message"),
                String::new()
            )),
...
fn test_kahn_tiebreak_mods_banning_each_other_v2_1_1() {
    ...
    let resolved = rz_core::resolve_iterative_sort(
        &unconflicted,
        &conflicted_events,
        &auth_context,
...
fn test_v2_1_1_cve_demotion_evasion() {
    ...
    let resolved_control = rz_core::resolve_iterative_sort(
        &unconflicted,
        &control_conflicted,
        &auth_context,
    ...
    let resolved_v21 = rz_core::resolve_iterative_sort(
        &unconflicted,
        &conflicted_events,
        &auth_context,
        rz_core::StateResVersion::V2_1,
    ...
    let resolved_v211 = rz_core::resolve_iterative_sort(
        &unconflicted,
        &conflicted_events,
        &auth_context,
        rz_core::StateResVersion::V2_1_1,
...
fn test_v2_1_flaw_concurrent_ban_evasion() {
    ...
    assert!(
        !resolved_v211.contains_key(&(rz_core::basespec::event_types::EventType::from("m.room.name"), String::new())),
...
fn test_missing_auth_diff_mainline_distortion() {
    ...
    let (resolved_buggy, _) = rz_core::resolve::resolve_iterative_sort_with_cache_and_deltas(
    ...
    let (resolved_fixed, _) = rz_core::resolve::resolve_iterative_sort_with_cache_and_deltas(
        unconflicted_state.clone(),
        conflicted_fixed,
        &events_map,
        None,
...
fn test_v2_1_1_ban_supplementation_return_path() {
    ...
    for ev in sorted_auth {
        if let Some(sk) = &ev.state_key {
            unconflicted.insert(
                (
                    rz_core::basespec::event_types::EventType::from(ev.event_type.as_str()),
                    sk.clone(),
                ),
                ev.event_id.clone(),
            );
...
fn test_process_pulled_event_with_rejected_missing_state() {
    ...
    let result = rz_core::resolve_state_maps(&state_maps, &auth_context, StateResVersion::V2_1_1);
...
fn auth_diff_context_event_scenario(version: StateResVersion) -> Option<String> {
    ...
    let completed = rz_core::compute_state_at_streaming_optimized(
        &["$merge"],
        &events,
        version,
        |id, update| {
