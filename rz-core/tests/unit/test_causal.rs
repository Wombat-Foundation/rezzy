...
fn causal_non_inclusion_proof_on_empty_set() {
    ...
    let (path, terminal_depth, root, count) = s.non_inclusion_proof(&d).unwrap();
    assert_eq!(path, [] as [rz_core::merkle::causal::CausalProofStep; 0]);
...
fn verify_causal_inclusion_rejects_tampered_sibling() {
    ...
    assert_ne!(path, [] as [rz_core::merkle::causal::CausalProofStep; 0]);
    path[0].hash[0] ^= 0xFF;
...
fn verify_causal_inclusion_rejects_extended_proof() {
    ...
    path.push(rz_core::merkle::causal::CausalProofStep {
        hash: [0xAA; 32],
        count: 0,
    });
    ...
    let (path, root, count) = s.inclusion_proof(&k).unwrap();
    let mut extended = vec![rz_core::merkle::causal::CausalProofStep {
        hash: [0xBB; 32],
        count: 1,
    }];
...
fn compress_inclusion_roundtrip_root_level_sibling() {
    ...
    assert_ne!(
        compressed,
        [] as [rz_core::merkle::causal::CompressedCausalStep; 0]
    );
...
fn compress_non_inclusion_on_empty_set() {
    ...
    assert_eq!(
        decompressed,
        [] as [rz_core::merkle::causal::CausalProofStep; 0]
    );
    assert!(verify_causal_non_inclusion(
        &d,
        terminal_depth,
