...
type SKey = (rz_core::basespec::event_types::EventType, String);
        ...
        for t in 0..threads {
            ...
            s.spawn(move || {
                ...
                while iter < ITER_COUNT {
                    ...
                    let problem = gen_problem(&mut rng, 1000 + iter * 13);
                    let safe =
                        rz_core::cdo::apply_cdo_filter(&problem.conflicted, &problem.auth_context);
...
fn dominated_winner_generator() {
    ...
    for i in 0u64..ITER_COUNT {
        ...
        let safe = rz_core::cdo::apply_cdo_filter(&p.conflicted, &p.auth_context);
