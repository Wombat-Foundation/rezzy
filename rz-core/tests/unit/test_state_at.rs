...
use rz_core::{verify_pagination, EventType, LeanEvent, PaginationViolation, StateResVersion};
...
fn test_topo_functions_ignore_federation_depth() {
    ...
    let depths = rz_core::compute_depths(&events_map);
    ...
    let state = rz_core::compute_state_at("D", &events_map, StateResVersion::V2, &String::new())
...
fn test_reverse_topo_order_correct_despite_inflated_depth() {
    ...
    let order = rz_core::reverse_topological_order("D", &events_map, |a: &String, b: &String| {
        a.cmp(b).reverse()
    });
...
{"event_id":"C","type":"m.room.message","sender":"@x:x","depth":3,"prev_events":["B"],"auth_events":["A","B","MISSING"]}
    ...
    let gaps = rz_core::find_missing_auth_events(&events_map, |_| false);
...
{"event_id":"B","type":"m.room.message","sender":"@x:x","depth":2,"prev_events":["A"],"auth_events":["A","IN_DB"]}
    ...
    let gaps = rz_core::find_missing_auth_events(&events_map, |id| id == "IN_DB");
...
fn test_compute_topo_positions_diamond() {
    ...
    let sorted = rz_core::compute_topo_positions(&events_map, |a: &String, b: &String| a.cmp(b));
...
fn test_resolve_merge_fast_path_hashed_mismatch() {
    use rz_core::state::at::{resolve_merge_fast_path_hashed, HashedState, LocalAuthCache};
...
{"event_id":"topic_event","type":"m.room.topic","state_key":"","sender":"@evil:example.com","depth":3,"content":{"topic":"Evil Topic"},"prev_events":["create_event"],"auth_events":["create_event"],"origin_server_ts":3000}
    ...
    let expected_hash = rz_core::state::lthash::LtHash::from_state(&merged.state);
...
{"event_id":"A","type":"m.room.create","state_key":"","sender":"@x:x","depth":1,"content":{"room_version":"10","creator":"@x:x"},"prev_events":[],"auth_events":[]}
    ...
    assert!(rz_core::compute_state_at(
        &"GHOST".to_string(),
        &events_map,
        StateResVersion::V2,
        &String::new(),
    ...
    let batch = rz_core::compute_state_at_batch(
        &["A", "GHOST"],
        &events_map,
        StateResVersion::V2,
        &String::new(),
...
fn test_state_computation_error_display() {
    let cyc = rz_core::StateComputationError::<&'static str>::CycleDetected;
    assert!(cyc.to_string().contains("Cycle detected"));

    let cb = rz_core::StateComputationError::<&'static str>::Callback("boom");
...
{"event_id":"B","type":"m.room.message","sender":"@x:x","depth":0,"prev_events":["A"],"auth_events":[]}
    ...
    rz_core::compute_state_at_streaming(
        &["A"],
        &events_map,
        StateResVersion::V2,
        |_, _| {},
...
{"event_id":"c2","type":"m.room.member","state_key":"@c2:x","sender":"@c2:x","depth":2,"content":{"membership":"join"},"prev_events":[],"auth_events":["c1"]}
    ...
    let mut unconflicted: rz_core::SharedState<String, String> = rz_core::SharedState::new();
    unconflicted.insert(
        (
            rz_core::basespec::event_types::EventType::from("m.room.member"),
            "@u:x".to_string(),
    ...
    let mut conflicted = rz_core::HashSet::new();
    conflicted.insert("c2".to_string());

    let diff = rz_core::compute_auth_chain_diff(&unconflicted, &conflicted, &events_map);
...
{"event_id":"A","type":"m.room.message","sender":"@x:x","depth":1,"prev_events":[],"auth_events":[]}
    ...
    let empty: &[&str] = &[];
    assert!(rz_core::compute_merge_base(empty, &events_map).is_none());
    assert_eq!(
        rz_core::compute_merge_base(&["A"], &events_map),
        Some(&"A".to_string())
    );
}
...
{"event_id":"B","type":"m.room.member","state_key":"@b:x","sender":"@b:x","depth":1,"content":{"membership":"join"},"prev_events":["R"],"auth_events":[]}
    ...
    assert_eq!(
        rz_core::compute_merge_base(&["A", "B"], &events_map),
        Some(&"R".to_string())
    );
}
...
{"event_id":"Y","type":"m.room.message","sender":"@x:x","depth":0,"prev_events":[],"auth_events":[]}
    ...
    let junctions = rz_core::compute_merge_bases(&["A", "B"], &events_map, 100);
    ...
    assert_eq!(
        rz_core::compute_merge_bases(&["A"], &events_map, 100),
        [] as [rz_core::MergeBase<&std::string::String>; 0]
    );

    // Zero step budget -> no traversal happens.
    assert_eq!(
        rz_core::compute_merge_bases(&["A", "B"], &events_map, 0),
        [] as [rz_core::MergeBase<&std::string::String>; 0]
    );

    // Disjoint tips -> no common ancestor, empty result.
    assert_eq!(
        rz_core::compute_merge_bases(&["X", "Y"], &events_map, 100),
        [] as [rz_core::MergeBase<&std::string::String>; 0]
    );
}
...
{"event_id":"E3","type":"m.room.message","sender":"@x:x","depth":0,"prev_events":["REMOTE"],"auth_events":[]}
    ...
    let extremities = rz_core::find_backward_extremities(&events_map, |id| id == "REMOTE");
...
{"event_id":"E3","type":"m.room.member","state_key":"@e3:x","sender":"@e3:x","depth":0,"content":{"membership":"join"},"prev_events":[],"auth_events":["REMOTE_AUTH"]}
    ...
    let missing = rz_core::find_missing_auth_events(&events_map, |id| id == "REMOTE_AUTH");
...
{"event_id":"D","type":"m.room.message","sender":"@x:x","depth":3,"prev_events":["B","C"],"auth_events":[]}
    ...
    let positions = rz_core::compute_topo_positions(&events_map, |a: &String, b: &String| a.cmp(b));
...
{"event_id":"D","type":"m.room.message","sender":"@x:x","depth":3,"prev_events":["B","C"],"auth_events":[]}
    ...
    let order = rz_core::reverse_topological_order(
        &"D".to_string(),
        &events_map,
...
{"event_id":"B","type":"m.room.message","sender":"@x:x","depth":2,"prev_events":["A"],"auth_events":[]}
    ...
    let violations = rz_core::verify_pagination(&events_map, &pages);
    assert!(violations
        .iter()
...
{"event_id":"D","type":"m.room.message","sender":"@x:x","depth":3,"prev_events":["B","C"],"auth_events":["A"]}
    ...
    let state = rz_core::compute_state_at(
        &"D".to_string(),
        &events_map,
        StateResVersion::V2,
        &String::new(),
...
{"event_id":"M","type":"m.room.member","state_key":"@creator:x","sender":"@creator:x","depth":2,"content":{"membership":"join"},"prev_events":["A"],"auth_events":["A"]}
    ...
    first.hash = rz_core::state::lthash::LtHash::from_state(&first.state);
    ...
    second.hash = rz_core::state::lthash::LtHash::from_state(&second.state);
    ...
    first2.hash = rz_core::state::lthash::LtHash::from_state(&first2.state);
    ...
    second2.hash = rz_core::state::lthash::LtHash::from_state(&second2.state);
...
{"event_id":"D","type":"m.room.name","state_key":"","sender":"@x:x","depth":4,"content":{"name":"room"},"prev_events":["C"],"auth_events":["A","B"]}
    ...
    let str_state =
        rz_core::compute_state_at("D", &events_map, StateResVersion::V2, &String::new()).unwrap();
    let interned_state = rz_core::compute_state_at(
        "D",
        &interned_map,
        StateResVersion::V2,
        &rz_core::InternedKey::default(),
    )
    .unwrap();
...
{"event_id":"C","type":"m.room.member","state_key":"@y:x","sender":"@x:x","depth":3,"content":{"membership":"join"},"prev_events":["B"],"auth_events":["A","B"]}
    ...
    let state = rz_core::compute_state_at("C", &interned_map, StateResVersion::V2, &InternId(0))
