...
fn test_invite_banned_user_rejected() {
    ...
    assert!(
        matches!(
            check_auth(&invite_banned, &state, rz_core::StateResVersion::V2_1, None),
...
fn test_invite_insufficient_power_level() {
    ...
    assert!(
        matches!(
            check_auth(&invite, &state, rz_core::StateResVersion::V2_1, None),
...
fn test_self_invite_rejected() {
    ...
    assert!(
        matches!(
            check_auth(&self_invite, &state, rz_core::StateResVersion::V2_1, None),
...
fn test_join_banned_user_rejected() {
    ...
    assert!(
        matches!(
            check_auth(&join_attempt, &state, rz_core::StateResVersion::V2_1, None),
            Err(AuthError::BannedUser { .. })
        ),
...
fn test_non_member_rejection() {
    ...
    let state: RoomState = RoomState::new();
    assert!(matches!(
        check_auth(&msg, &state, rz_core::StateResVersion::V2_1, None),
...
fn test_join_self_only() {
    ...
    let state: RoomState = RoomState::new();
    assert!(matches!(
        check_auth(&join, &state, rz_core::StateResVersion::V2_1, None),
        Err(AuthError::NotMember { .. })
    ));
}
...
fn test_iterative_auth_chain_rejects_foreign_room_auth_event() {
    ...
    // the V12-only rule.
    let room_a = rz_core::RoomId::new("!room_a:example.com");
    let room_b = rz_core::RoomId::new("!room_b:example.com");
    ...
    let (accepted, rejected) = check_auth_chain(
        ...
        &RoomState::new(),
        rz_core::basespec::rezzy_types::StateResVersion::V2,
    );
    ...
    assert!(
        matches!(
            &rejected[0].1,
            rz_core::auth::AuthError::ForeignRoomEvent { event_id, auth_event_id, .. }
...
fn test_iterative_auth_chain_rejects_untagged_auth_event_once_citing_side_populated() {
    ...
    let room_a = rz_core::RoomId::new("!create");
    ...
    assert!(
        matches!(
            &rejected[0].1,
            rz_core::auth::AuthError::ForeignRoomEvent { event_id, auth_event_id, actual: None, .. }
...
{"event_id":"$bob_msg","type":"m.room.message","sender":"@bob:x.com","depth":4,"origin_server_ts":1004,"content":{"body":"I am banned"},"prev_events":["$ban_bob"],"auth_events":["$ban_bob","$pl"]}
    ...
    let (accepted, rejected) =
        check_auth_chain(&events, &RoomState::new(), rz_core::StateResVersion::V2_1);
...
{"event_id":"$downstream","type":"m.room.message","sender":"@alice:x.com","depth":5,"origin_server_ts":1005,"content":{"body":"hi"},"prev_events":["$bad_ban"],"auth_events":["$create","$alice_join","$bad_ban"]}
    ...
    let (accepted, rejected) =
        check_auth_chain(&events, &RoomState::new(), rz_core::StateResVersion::V2);
...
"#,
    ...
    let res = check_auth(&events[0], &state, rz_core::StateResVersion::V2, None);
    assert!(
...
"#,
    ...
    let res2 = check_auth(&events2[0], &state, rz_core::StateResVersion::V2, None);
    assert!(
        res2.is_ok(),
...
fn test_v2_pl_wipeout_vulnerability() {
    ...
    let result = check_auth(&state_event, &state, rz_core::StateResVersion::V2, None);
    assert!(
        result.is_err(),
    ...
    let result = check_auth(&fix_pl, &state, rz_core::StateResVersion::V2, None);
    assert!(
        result.is_err(),
...
fn test_msc4289_v2_1_creator_immune_to_pl_wipeout() {
    ...
    let result = check_auth(&state_event, &state, rz_core::StateResVersion::V2_1, None);
    assert!(
        result.is_ok(),
...
fn test_msc4289_creator_implicit_power_level() {
    ...
    assert!(
        matches!(
            check_auth(&normal_kick, &state, rz_core::StateResVersion::V2_1, None),
            Err(AuthError::InsufficientPowerLevel {
                required: 50,
                actual: 0,
                ..
...
"#,
    ...
    let res = check_auth(creator_event, &state, rz_core::StateResVersion::V2, None);
    assert!(
        res.is_ok(),
    ...
    let res = check_auth(normal_event, &state, rz_core::StateResVersion::V2, None);
    assert!(
        matches!(
            res,
...
fn test_ban_insufficient_power_level() {
    ...
    let result = check_auth(&ban, &state, rz_core::StateResVersion::V2_1, None);
    assert!(
        matches!(
            result,
            Err(AuthError::InsufficientPowerLevel {
...
fn test_kick_insufficient_power_level() {
    ...
    let result = check_auth(&kick, &state, rz_core::StateResVersion::V2_1, None);
    assert!(
        matches!(
            result,
            Err(AuthError::InsufficientPowerLevel {
...
fn test_join_rules_not_member_invite_only() {
    ...
    let result = check_auth(&join_attempt, &state, rz_core::StateResVersion::V2_1, None);
    assert!(
        matches!(
            result,
            Err(AuthError::NotMember {
...
fn test_membership_rules_fallback() {
    ...
    let result = check_auth(&unknown, &state, rz_core::StateResVersion::V2_1, None);
...
fn test_invite_already_joined_user_rejected() {
    ...
    let result = check_auth(&invite, &state, rz_core::StateResVersion::V2_1, None);
    assert!(
        result.is_err(),
...
fn test_unstable_msc3757_owned_state_key_rejected_when_sender_mismatch() {
    ...
    let result = check_auth(&owned_event, &state, rz_core::StateResVersion::V2_1, None);
    assert!(
        result.is_err(),
...
fn test_self_leave_rejected_when_already_left() {
    ...
    let result = check_auth(&leave, &state, rz_core::StateResVersion::V2_1, None);
    assert!(
        result.is_err(),
...
fn test_third_party_invite_rejected_when_target_banned() {
    use rz_core::basespec::event_types::{
...
fn test_third_party_invite_rejected_when_token_missing() {
    use rz_core::basespec::event_types::{M_ROOM_MEMBER, M_ROOM_POWER_LEVELS};
    let mut state = RoomState::new();
    state.insert(
        (M_ROOM_POWER_LEVELS.into(), String::new()),
        make_event(
...
fn test_third_party_invite_override_is_ignored() {
    ...
    let result =
        rz_core::auth::check_auth(&tpi_event, &state, rz_core::StateResVersion::V2_1, None);
    assert!(
        matches!(
            result,
            Err(rz_core::auth::AuthError::InsufficientPowerLevel { .. })
        ),
...
fn test_malformed_third_party_invite_presence() {
    use rz_core::basespec::event_types::{M_ROOM_CREATE, M_ROOM_MEMBER, M_ROOM_POWER_LEVELS};
    let mut state = RoomState::new();
    state.insert(
        (M_ROOM_CREATE.into(), String::new()),
        make_event(
    ...
    let result =
        rz_core::auth::check_auth(&invite_event, &state, rz_core::StateResVersion::V2_1, None);
    assert!(
        matches!(result, Err(rz_core::auth::AuthError::InvalidSyntax(_))),
...
impl rz_core::EventVerifier<String> for PassThroughVerifier {}

/// A verifier that rejects on `verify_event_id_hash`.
struct RejectEventIdHash;
impl rz_core::EventVerifier<String> for RejectEventIdHash {
...
impl rz_core::EventVerifier<String> for RejectSignatures {
...
impl rz_core::EventVerifier<String> for RejectContentHash {
...
impl rz_core::EventVerifier<String> for RejectThirdPartyInvite {
...
fn test_third_party_invite_default_pl_without_power_levels() {
    use rz_core::basespec::event_types::{M_ROOM_MEMBER, M_ROOM_THIRD_PARTY_INVITE};
    ...
    let result = rz_core::auth::check_auth(&tpi, &state, rz_core::StateResVersion::V2, None);
    assert!(
        result.is_ok(),
...
fn test_member_event_missing_state_key_rejected() {
    ...
    let result = rz_core::auth::check_auth(&malformed, &state, rz_core::StateResVersion::V2, None);
    assert!(
        result.is_err(),
...
fn test_prev_events_exceeds_max_rejected() {
    ...
    let result = rz_core::auth::check_auth(&event, &state, rz_core::StateResVersion::V2, None);
...
"#,
    ...
    let res = check_auth(&events[0], &state, rz_core::StateResVersion::V2_1, None);
    assert!(
        res.is_err(),
...
fn test_pl_missing_create_event_returns_error() {
    ...
    let result = check_auth(&events[0], &state, rz_core::StateResVersion::V2_1, None);
    assert!(
...
fn test_auth_missing_create_event_in_v2_room_state_with_context() {
    ...
    let provider: rz_core::HashMap<String, LeanEvent> = rz_core::HashMap::new();
    let event = make_event(
        "$pl",
        "m.room.power_levels",
        Some(""),
...
fn test_forward_extremity_validation_valid() {
    ...
    let result =
        validate_forward_extremity(&event, &state, &state, rz_core::StateResVersion::V2_1, None);
...
fn test_warn_unexpected_auth_events_v12_create() {
    ...
    let create_event = make_event(
        "$c",
        rz_core::basespec::event_types::M_ROOM_CREATE,
        Some(""),
        "@alice:x.com",
        json!({}),
    );
    ...
    warn_unexpected_auth_events(&event, &auth_context, rz_core::StateResVersion::V2_1);
}
...
fn test_warn_unexpected_auth_events_unexpected_type() {
    ...
    warn_unexpected_auth_events(&event, &auth_context, rz_core::StateResVersion::V1);
}
...
fn test_warn_unexpected_auth_events_valid() {
    ...
    let member_event = make_event(
        "$j",
        rz_core::basespec::event_types::M_ROOM_MEMBER,
        Some("@alice:x.com"),
        "@alice:x.com",
        json!({"membership": "join"}),
    );
...
fn test_pl_v10_plus_users_contains_non_integer_rejected() {
    let cases = vec![
        ("10", rz_core::StateResVersion::V2),
        ("12", rz_core::StateResVersion::V2_1),
    ];
    for (version_str, state_res) in cases {
        ...
        let pl = make_event(
            "$pl",
            rz_core::basespec::event_types::M_ROOM_POWER_LEVELS,
            Some(""),
            "@admin:x.com",
            json!({
                "users": {
...
fn test_domain_parsing_helpers() {
    use rz_core::basespec::rezzy_types::{domain_matches, extract_domain};
...
fn test_rule_3_m_federate_false_cross_domain_rejected() {
    use rz_core::basespec::event_types::M_ROOM_MEMBER;
    let mut state = RoomState::new();
    let create_ev = make_event(
        "$c",
        M_ROOM_CREATE,
...
fn test_rule_11_redaction_sufficient_pl_different_domain_allowed() {
    use rz_core::basespec::event_types::M_ROOM_POWER_LEVELS;
...
fn test_rule_2_1_duplicate_auth_event_pair_rejected() {
    ...
    let mut provider = rz_core::HashMap::new();
...
fn test_room_id_conversions_and_borrow_impls() {
    let via_new = rz_core::RoomId::new("!room:example.com");
    let via_from_str: rz_core::RoomId = "!room:example.com".into();
    let via_from_string: rz_core::RoomId = String::from("!room:example.com").into();
...
fn test_interned_key_as_lean_event_state_key() {
    let via_new = rz_core::InternedKey::new("@bob:example.com");
    let via_from_str: rz_core::InternedKey = "@bob:example.com".into();
    let via_from_string: rz_core::InternedKey = String::from("@bob:example.com").into();
    assert_eq!(via_new, via_from_str);
    assert_eq!(via_new, via_from_string);
    ...
    let a: rz_core::InternedKey = "a".into();
    let b: rz_core::InternedKey = "b".into();
    assert!(a < b);
...
fn test_msc4242_prev_state_events_limit_in_check_auth() {
    ...
    let res = rz_core::auth::check_auth(&ev, &state, StateResVersion::V2_2, None);
    assert!(matches!(
        res,
...
fn test_v2_2_auth_context_skips_auth_events_selection_rules() {
    let mut provider: rz_core::HashMap<String, LeanEvent> = rz_core::HashMap::new();
