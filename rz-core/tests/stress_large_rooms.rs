...
fn test_ruma_bootstrap_auth_chain() {
    use rz_core::auth::{check_auth_chain, RoomState};
...
fn test_large_room_10k_subgraph_bounded() {
    ...
    let bounded = rz_core::compute_v2_1_conflicted_subgraph_bounded(&map, &pl_events, Some(5));
    assert!(
        !bounded.subgraph.is_empty(),
        "Bounded subgraph should contain events"
    );
    // Unbounded should be >= bounded
    let full = rz_core::compute_v2_1_conflicted_subgraph_bounded(&map, &pl_events, None);
    assert!(
...
fn test_unredacted_spam_storm_v2_1_1() {
    ...
    let resolved_lattice = rz_core::resolve_semilattice_fold(
...
fn verify_spam_storm_results(
    events: &[LeanEvent],
    resolved_v2: &imbl::OrdMap<(rz_core::basespec::event_types::EventType, String), String>,
    resolved_v21: &imbl::OrdMap<(rz_core::basespec::event_types::EventType, String), String>,
    resolved_v211: &imbl::OrdMap<(rz_core::basespec::event_types::EventType, String), String>,
    resolved_lattice: &imbl::OrdMap<(rz_core::basespec::event_types::EventType, String), String>,
    durs: (
        std::time::Duration,
        std::time::Duration,
        std::time::Duration,
    ...
    let n_components = {
        ...
        let id_to_index = rz_core::index_by_event_id(events.iter());
